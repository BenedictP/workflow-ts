...
export type RuntimeLifecycleMode = 'always-on' | 'pause-when-backgrounded';
...
const isDirectlyComparableSetValue = (value: unknown): boolean => {
  return (
    value === null || value === undefined || typeof value === 'function' || !isObjectLike(value)
  );
};
...
export const createPropsSnapshot = (props: unknown): PropsSnapshot => {
...
export interface WorkflowRuntimeOptions<O> {
...
export const useManagedWorkflowRuntime = <P extends AllowedProp, S, O, R>(
  ...
  const pendingDisposalsRef = useRef(
    new Map<WorkflowRuntime<P, S, O, R>, ReturnType<typeof setTimeout>>(),
  );
  const workflowRef = useRef(options.workflow);
  const runtimeIdentityRef = useRef(options.runtimeIdentity);

  const lifecycle = options.lifecycle ?? 'always-on';
  const shouldBeActive =
    lifecycle === 'pause-when-backgrounded' ? (options.isActive ?? true) : true;
  const previousActiveRef = useRef(shouldBeActive);
  const workflowChanged = workflowRef.current !== options.workflow;
  const runtimeIdentityChanged = !Object.is(runtimeIdentityRef.current, options.runtimeIdentity);
  ...
  // 4. Runtime identity changed (e.g. persistence key/config changes)
  ...
  if (needsNewRuntime) {
    ...
    options.runtimeRef.current = options.createRuntime(
      options.workflow,
  ...
  runtimeIdentityRef.current = options.runtimeIdentity;

  const runtime = options.runtimeRef.current;
  const scheduleDispose = useCallback(
    (runtimeToDispose: WorkflowRuntime<P, S, O, R>) => {
    ...
    for (const [type, handler] of Object.entries(handlers ?? {})) {
      ...
      const unsubscribe = runtime.on(
        ...
        ((output: Extract<O, { type: OutputType }>) => {
          ...
          (latestHandler as (output: Extract<O, { type: OutputType }>) => void)(output);
