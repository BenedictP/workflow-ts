...
- React CJS builds no longer emit `import.meta` warnings, and contributor docs now use `pnpm run ci` so the repository CI script is invoked correctly.
...
- `usePersistedWorkflow` development-warning detection now uses explicit environment signals (`__DEV__`, `NODE_ENV`) and defaults to non-warning mode when unknown.
